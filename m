...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id E59EB759EE
	for <lists+linux-integrity@lfdr.de>; Thu, 25 Jul 2019 23:58:47 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726766AbfGYV6r (ORCPT <rfc822;lists+linux-integrity@lfdr.de>);
        Thu, 25 Jul 2019 17:58:47 -0400
Received: from mx1.redhat.com ([209.132.183.28]:45374 "EHLO mx1.redhat.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S1726380AbfGYV6q (ORCPT <rfc822;linux-integrity@vger.kernel.org>);
        Thu, 25 Jul 2019 17:58:46 -0400
Received: from smtp.corp.redhat.com (int-mx07.intmail.prod.int.phx2.redhat.com [10.5.11.22])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mx1.redhat.com (Postfix) with ESMTPS id 2F643C04959E;
        Thu, 25 Jul 2019 21:58:46 +0000 (UTC)
Received: from localhost (ovpn-116-52.gru2.redhat.com [10.97.116.52])
        by smtp.corp.redhat.com (Postfix) with ESMTP id 3933F1001B33;
        Thu, 25 Jul 2019 21:58:44 +0000 (UTC)
Date:   Thu, 25 Jul 2019 18:58:43 -0300
...
Message-ID: <20190725215843.GP6134@rhlt>
...
X-Greylist: Sender IP whitelisted, not delayed by milter-greylist-4.5.16 (mx1.redhat.com [10.5.110.31]); Thu, 25 Jul 2019 21:58:46 +0000 (UTC)
...
> @@ -1,4 +1,4 @@
> -SUBDIRS =3D src
> +SUBDIRS =3D src tests
>  dist_man_MANS =3D evmctl.1
> =20
>  doc_DATA =3D  examples/ima-genkey-self.sh examples/ima-genkey.sh example=
s/ima-gen-local-ca.sh
...
> @@ -0,0 +1,14 @@
> +check_SCRIPTS =3D
> +TESTS =3D $(check_SCRIPTS)
> +
> +check_SCRIPTS +=3D ima_hash.test ima_verify.test ima_sign.test
> +
> +# ima_verify depends on results of ima_hash
> +ima_verify.log: ima_sign.log

Small nit: comment and code don't match, ima_hash vs ima_sign :).=20
...
> +
> +declare -i pass=3D0 fail=3D0 skip=3D0
> +
> +_require() {
> +  RET=3D
> +  for i; do
...
> +    0)  pass+=3D1 ;;
> +    77) skip+=3D1 ;;
> +    99) fail+=3D1; exit_early 1 ;;
> +    *)  fail+=3D1; exit_early 2 ;;
> +  esac
> +}
> +
> +neg() {
> +  MODE=3D-
> +  HOWFAILED=3Dproperly
> +  eval "$@"
> +  case $? in
> +    0)  fail+=3D1; exit_early 3 ;;
> +    77) skip+=3D1 ;;
> +    99) fail+=3D1; exit_early 4 ;;
> +    *)  pass+=3D1 ;;
> +  esac
> +# Restore defaults
> +  MODE=3D+
...
> +
> +_evmctl_catch() {
> +  local ret=3D$1 out=3D$2 cmd=3D$3 for=3D$4 del=3D${@:5}
> +
...
> +	O =3D IMA-CA
> +	CN =3D IMA/EVM certificate signing key
> +	emailAddress =3D ca@ima-ca
> +
> +	[ v3_ca ]
...
> +    log openssl req -verbose -new -nodes -utf8 -sha1 -days 10000 -batch =
-x509 \
> +      -config test-ca.conf \
> +      -newkey rsa:$m \
...
> +    log openssl pkey -in test-$algo-$param.key -out test-$algo-$param.pu=
b -pubout
> +done
> +
...
> +# Check with constant
> +check_const() {
> +  local a=3D$1 p=3D$2 h=3D$3 f=3D$4
> +
> +  cmd=3D(evmctl ima_hash -v ${ENGINE:+--engine $ENGINE} -a $a --xattr-us=
er $f)
> +  echo $YELLOW$MODE "${cmd[@]}" $NORM
> +  eval "${cmd[@]}" >$a.out 2>&1
> +  _evmctl_catch $? $a.out ima_hash $a $f || return
> +
> +  if ! getfattr -n user.ima -e hex $f | grep -qx user.ima=3D$p$h; then
> +    red_if_pos
> +    echo "Did not find expected hash value for $a:"
> +    echo "    user.ima=3D$p$h"
> +    echo ""
...
> +
> +check() {
> +  local a=3D$1 p=3D$2 h=3D$3
> +  local f=3D$a-hash.txt
> +
> +  rm -f $f
> +  touch $f
> +  cmd=3D(openssl dgst ${ENGINE:+--engine $ENGINE} -$a $f)
> +  echo $BLUE - "${cmd[@]}" $NORM
> +  h=3D$(set -o pipefail; eval "${cmd[@]}" 2>/dev/null | cut -d' ' -f2)
...
> +keyid() {
> +  id=3D$(openssl x509 ${ENGINE:+-engine $ENGINE} \
...
> +
> +_ima_sign() {
> +  local k=3D$1 a=3D$2 f=3D$3
> +
> +  cmd=3D(evmctl ima_sign ${ENGINE:+--engine $ENGINE} -a $a \
...
> +
> +check_rsa() {
> +  local k=3Dtest-rsa1024 a=3D$1 p=3D$2
> +  local f=3D$a.txt
> +
...
> +  fi
> +  touch $f
> +  cmd=3D(openssl dgst -$a -sign $k.key -hex $f)
> +  echo $BLUE - "${cmd[@]}" $NORM
> +  sig=3D$(set -o pipefail; eval "${cmd[@]}" 2>/dev/null | cut -d' ' -f2)
> +  if [ $? -ne 0 ] && _ispos; then
> +    echo "$a ($k.key) test is skipped (openssl cannot sign with $a+$k.ke=
y)"
> +    rm -f $f
> +    return 77
> +  fi
...
> +  if ! getfattr -n user.ima -e hex $f | grep -qx user.ima=3D$p$sig; then
> +    red_if_pos
> +    echo "Did not find expected hash value for $a:"
> +    echo "    user.ima=3D$p$sig"
> +    echo ""
> +    echo "Actual output below:"
> +    getfattr -n user.ima -e hex $f | sed 's/^/    /'
> +    norm_if_pos
...
> +  if ! openssl pkey ${ENGINE:+-engine $ENGINE} -in $k.key >/dev/null 2>&=
1; then
...
> +  if ! getfattr -n user.ima -e hex $f | grep -q ^user.ima=3D$p; then
> +    red_if_pos
> +    echo "Signature prefix does not match for $a ($k):"
> +    echo "Expected:  user.ima=3D$p..."
> +    echo ""
> +    echo "Actual output below:"
> +    getfattr -n user.ima -e hex $f | sed 's/^/    /'
> +    norm_if_pos
...
> +    return 77
> +  fi
> +  if ! openssl dgst ${ENGINE:+--engine $ENGINE} -$a /dev/null >/dev/null=
 2>&1; then
...
> +  fi
> +
> +  cmd=3D(evmctl ima_verify ${ENGINE:+--engine $ENGINE} -v -k $k --xattr-=
user $f)
> +  echo $YELLOW$MODE "${cmd[@]}" $NORM
> +  eval "${cmd[@]}" >$f.out 2>&1
...
> +
> +_enable_gost_engine
> +wk=3Dtest-gost2012_512-B.cer # first wrong key
> +for k in gost2012_256-A gost2012_256-B gost2012_256-C \
> +         gost2012_512-A gost2012_512-B; do
> +  tmp=3D${k#gost2012_}
> +  bits=3D${tmp%-?}
> +  wbits=3D$(( $bits =3D=3D 256 ? 512 : 256 ))
> +  curve=3D${k#*-}
> +  a=3Dmd_gost12_$bits   # proper hash algo
> +  f=3D$a${curve,,}.txt  # file signed in ima_sign test
> +  k=3Dtest-$k.cer
> +  wwk=3Dtest-gost2012_$wbits-$curve.cer # key with wrong bit length
> +
> +  pos check $k     $a $f
> +  pos check $wk,$k $a $f
> +  pos check $k,$wk $a $f
> +  neg check $wk    $a $f
> +  neg check $wwk   $a $f
> +  wk=3D$k # previous key is always wrong
> +done
> +
> +_report_exit
> --=20
...

